...
class Request {
  ...
  execute(callback) {
    ...
    /** @type {module:LeagueWrapper~Cache} */
    const cache = self.api.options.cache;

    const cacheKey = self.options.cache.prepareKey(self.options.params, self.options.query);

    cache.get(cacheKey).then(function(data) {
      if (data) {
        return Promise.resolve(convert(data, self.options.returnType));
      }
          ...
          const request = client.get(
            ...
            function (data, response) {
              ...
              if (response.statusCode === 200) {
                ...
                cache.set(cacheKey, convertedData, self.options.cache);
